...
online_pitch_configDys=conf/online_pitch_dysarthric.conf
...
if [ -f $data/segments ]; then
  ...
  for JOB in $(seq $nj); do
     ...
     if [[ $line =~ M0.* || $line =~ F0.*  ]]; then
        ...
        config_pitch=$online_pitch_configDys
     else
        config=$mfcc_config
        config_pitch=$online_pitch_config
     fi

     mfcc_feats="ark:compute-mfcc-feats $vtln_opts --verbose=2 --config=$config scp,p:$logdir/wav_${name}.$JOB.scp ark:- |"
     pitch_feats="ark,s,cs:compute-and-process-kaldi-pitch-feats --verbose=2 --config=$config_pitch scp,p:$logdir/wav_${name}.$JOB.scp ark:- |"
