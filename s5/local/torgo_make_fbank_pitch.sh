...
fbank_configDys=conf/fbank_dysarthric.conf
...
if [ -f $data/segments ]; then
  ...
  for JOB in $(seq $nj); do
	...
	config_pitch=$pitch_config
     ...
     fbank_feats="ark:compute-fbank-feats $vtln_opts --verbose=2 --config=$config scp,p:$logdir/wav_${name}.$JOB.scp ark:- |"
     ...
     $cmd $logdir/make_fbank_pitch_${name}.$JOB.log \
       paste-feats --length-tolerance=$paste_length_tolerance "$fbank_feats" "$pitch_feats" ark:- \| \
       copy-feats --compress=$compress ark:- \
         ark,scp:$fbank_pitch_dir/raw_fbank_pitch_${name}.$JOB.ark,$fbank_pitch_dir/raw_fbank_pitch_${name}.$JOB.scp \
        || exit 1;
